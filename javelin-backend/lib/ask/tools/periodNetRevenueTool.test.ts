...
import { PERIOD_NET_REVENUE_INPUT_SCHEMA as SCHEMA } from './periodNetRevenueTool';

describe('PERIOD_NET_REVENUE_INPUT_SCHEMA', () => {
  ...
  it('rejects malformed date format', () => {
    expect(() =>
      SCHEMA.parse({ start: '2026-1-1', end: '2026-3-31' }),
    ).toThrow();
    expect(() =>
      SCHEMA.parse({ start: '01/01/2026', end: '03/31/2026' }),
    ).toThrow();
  });

  it('rejects calendar-invalid dates that pass the regex', () => {
    expect(() =>
